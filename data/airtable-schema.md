...
| `InStock` | Checkbox | |
| `ImageUrl` | URL | תמונת מוצר אחת לכל קטגוריה, מוגשת מ-CDN. נקראת בדפדפן בלבד — בלוח הבקרה ובאפליקציה. **אף תהליך ב-n8n אינו נוגע בה** |

`ImageUrl` נוסף אחרי שכל התהליכים כבר עבדו, וזו הסיבה שאפשר היה להוסיף
אותו בלי לשנות ולו תהליך אחד: הוא נקרא רק בצד הלקוח, דרך אותה קריאת
`list` על טבלת Products שכבר הייתה קיימת.
